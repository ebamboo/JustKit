...
JustKit 本质上是一个“总结型 App”：它不是单一业务产品，而是把 iOS 开发中可复用的工具、组件和经验方案沉淀到一个可运行的示例工程中。后续 AI agents 接手时，应优先把它理解为 **工具库 + 示例 App + 经验手册** 的组合体。

项目最重要的两条主线是：

- **通用工具（`Source/Utils/`）**：项目中复用价值最高、迁移成本最低的部分。这里的代码应尽量保持轻量、独立、少依赖，不依赖第三方库，适合直接复制到其他 iOS 项目中使用。新增基础扩展、通用 View、布局、存储、生命周期管理、闭包化适配等能力时，优先考虑放在这里。
- **项目工具（`Source/Common/`）**：面向真实项目场景的组件封装，允许依赖项目级第三方库或业务习惯，例如 Alamofire、SDWebImage、MBProgressHUD。这里的代码强调“拿来稍作适配即可用”，适合放网络层、SSE、图片网格、HUD 等偏业务基础设施的能力。

第三条主线是 **经验方案（`Source/Module/ExperienceTest/`）**。它主要用于沉淀行业内常见或特定技术场景的完整做法，例如视频播放、媒体浏览、嵌套滚动、键盘处理、文件预览、系统分享、自定义转场、相机旋转等。经验方案不一定都要抽象成通用工具，但应尽量保留清晰的演示入口和可阅读的实现路径，方便以后复制、改造或继续提炼。

修改规则上，AI agents 应遵循以下优先级：

- 如果是无第三方依赖、跨项目可复用的基础能力，放入 `Source/Utils/`。
- 如果是依赖项目技术栈或带业务基础设施性质的能力，放入 `Source/Common/`。
- 如果是某个具体 UI/交互/技术场景的完整实践，放入 `Source/Module/ExperienceTest/` 或对应演示模块。
- 新增工具或组件时，尽量同步补充一个最小可运行演示，让 App 仍然承担“工具说明书”的作用。
- 保持现有 Swift/UIKit 风格，中文注释可用于解释设计意图和使用场景，但避免把简单代码注释得过满。
